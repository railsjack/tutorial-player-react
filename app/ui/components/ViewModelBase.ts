...
import { getReduxValue, setReduxValueWithKey } from '../utils/store';
...
const useViewModel = (viewModel: any) => {
  return (params: any = {}) => {
    ...
    viewModel.observableVariables.map((obVariable: any) => {
      ...
      // if (observeValue != null && ((typeof observeValue).match(/string|number/) || observeValue.constructor.name === 'Array')) {
    ...
    if (observeValues && observeValues.length > 0) {
      // console.log('observeValues', observeValues);
      // eslint-disable-next-line react-hooks/rules-of-hooks
...
export { useViewModel, ObserveReducer, Observable };
